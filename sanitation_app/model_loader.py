"""
sanitation_app/model_loader.py
Loads the YOLO11s sanitation model and runs inference.
Place your model file at:  models/sanitation/best.pt
"""
...
MODEL_PATH = BASE_DIR / "models" / "sanitation" / "best.pt"

# ── Class config ──────────────────────────────────────────────────────────────
CLASS_NAMES  = ["cockroach", "trash", "wet_surface"]

# Risk weight per class (used for hygiene score)
CLASS_WEIGHTS = {
    "cockroach":   3.0,   # cockroach = always serious
    "trash":       2.5,
    "wet_surface": 1.5,
}
...
CLASS_COLORS = {
    "cockroach":   (0,   60,  220),   # red-ish
    "trash":       (0,  165,  255),   # orange
    "wet_surface": (200, 180,   0),   # teal
}

CONF_THRESHOLD = 0.1


class SanitationModelLoader:
    _instance = None
    ...
    def _load(self):
        try:
            ...
            if MODEL_PATH.exists():
                ...
                print(f"[SanitationLoader] Model loaded from {MODEL_PATH}")
            else:
                print(f"[SanitationLoader] WARNING – model not found at {MODEL_PATH}")
        except ImportError:
            print("[SanitationLoader] ultralytics not installed.")
    ...
    def predict(self, image_array: np.ndarray) -> dict:
        ...
        # ── Hygiene risk score (0–100, lower = safer) ─────────────────────────
        ...
        return {
            ...
            "annotated_image": annotated,   # numpy RGB
    ...
    def _compute_risk_score(self, detections: list, total_pixels: int) -> int:
        ...
        score = 0.0
        for d in detections:
            w = CLASS_WEIGHTS.get(d["label"], 1.0)
            conf_factor = d["confidence"] / 100
        # Each cockroach detection alone should push into moderate/critical
            score += w * conf_factor * 50
...
# Singleton
model_loader = SanitationModelLoader()
